...
export async function validateSupabaseConnection(
  ...
  try {
    ...
    const projectRes = await net.fetch(`https://api.supabase.com/v1/projects/${projectRef}`, {
    ...
    // Fetch API keys
    const keysRes = await net.fetch(`https://api.supabase.com/v1/projects/${projectRef}/api-keys`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
...
export async function supabaseInsert(
  ...
  try {
    const res = await net.fetch(`${getProjectUrl()}/rest/v1/${collection}`, {
      method: 'POST',
      headers: {
        ...authHeaders(),
...
export async function supabaseUpload(
  ...
  try {
    const res = await net.fetch(
      `${getProjectUrl()}/storage/v1/object/videos/${remotePath}`,
      {
        method: 'POST',
        headers: {
          ...authHeaders(),
    ...
    const publicUrl = `${getProjectUrl()}/storage/v1/object/public/videos/${remotePath}`
